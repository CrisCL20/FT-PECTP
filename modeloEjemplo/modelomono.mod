...
/* Funcion objetivo */
minimize FO1 : F[1]     ; # para minimizar cada objetivo por separado
...
/* Objetivos */
 ...
 O1 : F[1] = sum {(i,j) in A, m in M, k in K} (c[i,j,m])*x[i,j,m,k] + sum{j in R, m in M, k in K} (cc[j,m]) * z[j,m,k]  ;  # funcion de costo
 O2 : F[2] = sum {j in R, m in M, k in K} tf[j,m]*z[j,m,k] ; # funcion de tasa de fallo
